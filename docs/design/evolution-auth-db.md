...
it answers with. So is the **authentication block** that packages the challenge
cycle (`AuthDb.SBB.authenticate/1`), and the `P-Asserted-Identity` a proved
identity now travels as — [DESIGN-AUTH.md](DESIGN-AUTH.md) §3 and §4, designed
here and moved there when they shipped.

Everything below assumes those rules and changes only where the verdict comes
from.
