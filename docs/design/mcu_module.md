...
| Discovery | a client can be handed the template list | `kelictl` never reads the registry | **gap, pre-existing** (`Kelix.Control.CLI.parse/1` is a catch-all passthrough). Fix: `kelictl mcu help` rendering the registry — FW-5 |
