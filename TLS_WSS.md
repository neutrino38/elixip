...
For the TLS listener internal design, see [docs/design/DESIGN-SIPSTACK.md#34-listeners](docs/design/DESIGN-SIPSTACK.md#34-listeners).
...
it will follow the same pattern as `SIP.Transport.TLSListener` but wrap the accepted
TLS socket in a WebSocket upgrade handshake before spawning a `SIP.Transport.WSS`
inbound instance.
