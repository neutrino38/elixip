...
defmodule UAS.RegisterExample do
  ...
  # ---------------------------------------------------------------------------
  # First REGISTER: challenge if unauthenticated, accept once credentials are
  # present. `check_registration_auth/1` returns :ok when no password is
  # configured (test-tool default — trust any well-formed Authorization).
  ...
  # ---------------------------------------------------------------------------
  # Registered: accept refreshes; finish when the dialog terminates (expiry /
  # un-REGISTER) or after a long idle guard.
  state registered do
    on_events do
      {:REGISTER, req, _trans_pid, dialog_pid} ->
        accept_registration(req, dialog_pid, expires: @granted_expires)
  ...
  defp check_registration_auth(req, opts \\ []) do
    has_auth = Map.has_key?(req, :authorization) or Map.has_key?(req, :proxyauthorization)
