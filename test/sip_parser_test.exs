defmodule SIPParserTest do
  use ExUnit.Case
  doctest SIPUriParser

  test "Parse an URI that has only a domain" do
		{ code, parsed_uri } = SIPUriParser.parse_sip_uri("sip:domain.fr")
		assert code == :ok
  ...
  test "Parse a simple URI" do
		...
		{ code, parsed_uri } = SIPUriParser.parse_sip_uri("sip:simple@domain.fr")
		assert code == :ok
  ...
  test "Parse a faulty SIP URI" do
		{ code, _parsed_uri } = SIPUriParser.parse_sip_uri("sipp:simple@domain.fr")
		assert code == :invalid_sip_uri_general
	end

  test "Parse an URI with transport parameter" do
		{ code, parsed_uri } = SIPUriParser.parse_sip_uri("sip:simple@domain.fr:5060;transport=TCP")
			assert code == :ok
			assert parsed_uri.userpart == "simple"
			assert parsed_uri.domain == "domain.fr"
  ...
  test "Parse an URI with <> and transport parameter" do
			{ code, parsed_uri } = SIPUriParser.parse_sip_uri("<sip:simple@domain.fr:5030>;transport=TCP;rport")
			assert code == :ok
			assert parsed_uri.userpart == "simple"
			assert parsed_uri.domain == "domain.fr"
  ...
  test "Parse an URI with a display name and several parameters" do
			{ code, parsed_uri } = SIPUriParser.parse_sip_uri("\"omé tür\" <sip:simple@domain.fr:50>;transport=TCP;rport")
			assert code == :ok
			assert parsed_uri.userpart == "simple"
			assert parsed_uri.domain == "domain.fr"
	...
	test "Parse an URI with a display name without spece" do
		{ code, parsed_uri } = SIPUriParser.parse_sip_uri("\"Site%20Arras%20POLE%20EMPLOI\"<sip:+33970260233@visioassistance.net>;tag=8075639")
		assert code == :ok
  ...
  test "Load and parse a REGISTER message" do
    { code, msg } = File.read("test/SIP-REGISTER.txt")
		...
		{ code, parsed_msg } = SIPMsgParser.parse(msg, fn code, errmsg, lineno, line ->
			IO.puts("\n" <> errmsg)
  ...
  test "Load and parse an invalid unREGISTER message" do
		...
		{ code, _parsed_msg } = SIPMsgParser.parse(msg, fn _code, _errmsg, lineno, line ->
			assert lineno == 1
  ...
  test "Parse a SIMPLE body with correct content length" do
		...
		bodylist = SIPMsgParser.parse_multi_part_body("application/sdp", data)
		assert code == :ok
		[ body ] = bodylist
